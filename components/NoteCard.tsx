...
import { getHighlightedText, formatRelativeTime, getContextSnippet } from '../utils';
...
export const NoteCard: React.FC<BookCardProps> = ({ 
  ...
  // Logic to find the most relevant entry snippet
  let previewContent = '';
  
  if (searchQuery.trim()) {
      // 1. Try to find the keyword in entries first
      const matchedEntry = book.entries.find(e => e.content.toLowerCase().includes(searchQuery.toLowerCase()));
      if (matchedEntry) {
          previewContent = getContextSnippet(matchedEntry.content, searchQuery);
      } else {
          // 2. If not in entries (maybe in title/tags), just show the start of the latest entry
          previewContent = book.entries.length > 0 ? getContextSnippet(book.entries[0].content, '') : '';
      }
  } else {
      // Default: Latest entry start
      previewContent = book.entries.length > 0 ? book.entries[0].content.substring(0, 150) : '';
  }

  return (
    <div 
      className={`
        relative group p-4 rounded-xl border transition-all duration-200 bg-white dark:bg-slate-800 flex items-start gap-3
        ${isSelected 
    ...
    >
      ...
      {/* Content Column - Flex 1 to take remaining space */}
      <div className="flex-1 min-w-0">
        ...
        {/* Row 3: Preview Content (With break-all for numbers) */}
        <p className="text-xs text-slate-600 dark:text-slate-300 line-clamp-3 mb-3 leading-relaxed tracking-wide opacity-90 break-all">
          {previewContent ? renderHighlight(previewContent) : <span className="italic text-slate-300 dark:text-slate-600">{t.noContent}</span>}
        </p>

        {/* Row 4: Footer (Entry Count & Date) */}
